...
import { getFrontendMode, getMarketNextPolicy } from '../../lib/market-config'
import { getBundleRecords, getPendingOverrides, patchMarketNextData } from '../../lib/data-store'
import { getIgnoredUpdateVersion, getLatestVersion, getUpdateIgnoreText, hasUpdate, isUpdateCheckDisabled, isUpdateIgnored } from '../../lib/update-policy'
import { activeBundle, expandedDependency, pendingBundleUninstalls } from '../../lib/dialogs'
import { analyzeVersions } from '../../lib/analyze-versions'
import { createLocalBundleRecord } from '../../lib/bundle-records'
import { ensureInstalledConfig, getConfigWriter } from '../../lib/config-writer'
import { formatEndpoint, getRegistryStatus, getRegistryStatusText } from '../../lib/registry-status'
...
export function useCard(props: DependencyCardProps) {
  ...
  const summaryText = computed(() => {
    ...
    return pickDescription(marketData.value?.manifest?.description, locale.value)
      ...
      || pickDescription(local.value?.package?.description, locale.value)
  })
